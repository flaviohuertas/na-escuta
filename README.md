...
**Dados de demonstração para explorar o sistema** (opcional, só em banco local): depois do `db:seed`, `npm run db:seed:demo` monta uma produtora fictícia completa — sete fornecedores, clientes, oportunidades em todas as etapas do funil, propostas, orçamentos, dois eventos com custos lançados (alguns categorias estouradas, um lançamento anulado). Cria também `produtor@naescuta.com.br` (mesma senha), que vê Comercial e Fornecedores mas **não** custo, margem nem financeiro. É idempotente e recusa rodar em banco que não seja `localhost`.

**Banco local de uma conexão só (`prisma dev`):** se as telas de Comercial, Fornecedores e Financeiro devolverem erro 500 com "Server has closed the connection" no terminal, adicione `DATABASE_POOL_MAX=1` ao `.env` e reinicie o `npm run dev`. Com Postgres de verdade (docker compose) não é preciso.
