...
**O que está implementado E testado** (171 testes unitários, `npm run test`, sem depender de Postgres — usam `fake-indexeddb` para simular IndexedDB e um `caches`/Service Worker falsos; 124 deles vieram das últimas rodadas: Painel, criar/editar evento (permissões, schema, formulário, propagação no pull), `EventWorkspace`, telas offline (`warm-routes`, `sw-warm`, `routes`), `AppLink`, detalhe por query, fallback offline, retentativa de conectividade e conflitos — vários com regressão comprovada, ou seja, falham sem a correção; ver seção 13):
...
**Validado contra Postgres real e navegador real** (Postgres 18.4 via `embedded-postgres`, sem Docker; Edge, build de produção — ver seção 11): a migration inicial aplica limpa; o seed roda; os **51 testes de integração** (`npm run test:integration`) passam — push/idempotência, conflitos (detecção, resolução, 409 com entidade, autorização), isolamento entre empresas, as consultas do Painel e criar/editar evento (permissões por papel, versão, corrida, propagação no pull). Os **12 testes E2E** (7 specs em `tests/e2e/`) passam, e foram repetidos em rodadas seguidas, inclusive com o banco acumulando dados: preparar evento, abrir e recarregar **sem rede**, criar tarefa/checklist/ocorrência offline **e abrir/recarregar o detalhe deles offline**, fallback de telas que exigem conexão, fechar e reabrir, reconectar sem duplicar, conflito entre dois dispositivos resolvido manualmente, sessão expirada, **criar e editar evento pela tela, a edição chegando a um aparelho já preparado (e ficando lá sem rede), o formulário sem conexão, e a recusa (tela e API) para quem tem papel restrito**. Login, `/painel`, o catálogo e o formulário de evento foram abertos no navegador (desktop e mobile). `npm run build`, `npm run lint` e `npm run typecheck` passam limpos.
...
src/app/
  ...
  (app)/                      shell autenticado — layout valida sessão e monta SyncProvider
    ...
    eventos/novo, eventos/[eventId]/editar   criar/editar evento (Server Component + formulário client; exige conexão; FORA do cache do SW)
  ...
  api/eventos/                POST criar, PATCH editar (401 sem sessão, 403 sem papel, 409 versão velha, 422 validação por campo)
...
| 16 (parcial) | Administração: criar/editar evento | **Implementado e validado** (ver seção 1). **Falta:** dar acesso ao evento a outras pessoas, administrar a equipe da empresa (papéis, convite, revogação) e excluir/arquivar evento |
...
Próximo passo recomendado: **acesso ao evento e equipe da empresa** (sem isso um evento novo só tem o criador e não há como pôr outra pessoa no sistema); depois o fluxo de aprovação e o CRM. O uso offline está fechado no que foi possível medir; o que falta nele é **testar em aparelho real** (modo avião de verdade, Safari/iOS, dispositivo compartilhado). Fora isso, seguir para o CRM/propostas (módulo 2), na ordem do enunciado — vale começar pelo **fluxo de aprovação** (`PendingApproval`, já no schema), que orçamentos, contratos e financeiro vão reusar.
...
5. **Sem tela para dar acesso ao evento a outras pessoas nem administrar a equipe** (próximo passo). Hoje um evento criado pela tela só tem o criador; outras pessoas só entram por seed/banco.
6. **`authorizeEventAccess` (usado por push/pull/bootstrap) não olha `User.isActive`.** Uma conta desativada com sessão ainda válida continua sincronizando até o vínculo com a empresa ser revogado. `createEvent` já recusa conta desativada; o sync ainda não.
...
**Placar (Edge, build de produção, Postgres real):** unitários 171 · integração 51 · E2E 12/12 (3 rodadas seguidas mais 2 no build final, todas 12/12). Os commits já publicados foram verificados isoladamente, em worktree limpo, antes do merge na `main`: typecheck, lint, 124 unitários, 20 de integração, build e 5 E2E.
