...
import { createClientViaUi, createOpportunityViaUi, randomCnpj } from "./helpers/crm";
...
test.describe("Comercial (CRM): clientes, funil e a oportunidade que vira evento", () => {
  test("cliente → oportunidade → funil → perder e reabrir → transformar em evento → arquivar", async ({ page }) => {
    test.slow(); // é um passeio longo (dezenas de telas): com a suíte toda em paralelo os 30 s padrão apertam
